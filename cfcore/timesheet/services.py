...
def _get_daily_checkin_logs(employee, date_day):
    ...
    try:
        ...
        frappe.log_error(
            f"Failed to fetch logs for {employee} on {date_day}: {e}",
            "Checkin Fetch Error",
        )
        return []
...
def _is_valid_log_sequence(logs, employee, date_day):
    ...
    expected_patterns = {
        2: [LogType.IN.value, LogType.OUT.value],
        4: [LogType.IN.value, LogType.OUT.value, LogType.IN.value, LogType.OUT.value],
    }
...
def _create_base_summary(date_day, first_log):
    ...
    return {"job_site": job_site, "day": date_day.strftime("%A"), "date": date_day}
...
def _get_or_create_wt(employee, from_date, to_date):
    wt = frappe.get_all(
        "Weekly Timesheet",
        filters={"employee": employee, "from_date": from_date, "to_date": to_date},
        limit=1,
    )
    if wt:
...
def apply_work_rate_snapshot(wt):
    if wt.use_override:
        wt.total_amount = wt.override_amount + wt.total_expenses
    elif wt.rate_mode == WorkRateMode.PER_DAY.value:
        wt.calculated_amount = wt.days_worked * wt.rate
    elif wt.rate_mode == WorkRateMode.PER_HOUR.value:
        wt.calculated_amount = wt.hours_worked * wt.rate


def do_snapshot_work_rate(wt):
    if wt.work_rate:
        work_rate = frappe.get_doc("Work Rate", wt.work_rate)
        wt.rate_name = work_rate.rate_name
        wt.rate_mode = work_rate.mode
        wt.rate = work_rate.rate


def calculate_expenses(wt):
    total_expenses = 0.0
    for expense in wt.expenses_detail:
        if expense.is_approved:
            total_expenses += expense.amount
    wt.total_expenses = total_expenses


def calculate_totals(wt):
    if wt.use_override:
        wt.total_amount = wt.override_amount + wt.total_expenses
    else:
